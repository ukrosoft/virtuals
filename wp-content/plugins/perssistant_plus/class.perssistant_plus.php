...
//        wp_enqueue_script('front_page_js', plugin_dir_url(__FILE__) . 'script.js', array('jquery'));
        ...
        add_action('wp_footer', array('perssistant_plus', 'scripts'));
    ...
    public static function testimonial_view($atts, $content = null)
    {
        //var_dump($_GET);
        $view_id = get_query_var('testimonial-view');
//var_dump($view_id);

        $current_testimonial = get_post($view_id);
        ?>
        <div class="container">
            <div class="row">
                <div class="col-xs-8">
                    <?php
                    var_dump($current_testimonial->post_content);
                    ?>
                </div>
                <div class="col-xs-4">
                    <?php
                    $image_popup_id = get_post_thumbnail_id($current_testimonial->ID);
                    if ($image_popup_id) {
                        $image_popup_url = wp_get_attachment_url($image_popup_id);
                    } else {
                        $image_popup_url = 'http://placehold.it/' . $atts['image_height'];
                    }

                    //var_dump($current_testimonial);

                    ?>
                    <img src="<?php echo $image_popup_url; ?>"
...
//        wp_enqueue_style('perssistant_plus');
    ...
    public static function our_clients_about_us($atts, $content = null)
    {
        //var_dump($atts);
        //var_dump($atts['per_page']);
        $review_count = (int)$atts['items'];
        //var_dump($review_count);
        $args = array(
            'posts_per_page' => $review_count,
            'category_name' => 'our-clients-about-us'
        ...
        $active = 'active';
        //var_dump($posts->post_count % 2);
        if ($posts->post_count % 2) {
            // var_dump($posts->post_count % 2);
            $indicators = round($posts->post_count / 2);
            //var_dump($indicators);
        } else {
            $indicators = round($posts->post_count / 2);
        ...
        if ($posts->have_posts()) {
            ++$postCount;
            ?>
            <h3 class="text-center"><?php echo $content; ?></h3>
            <div class="col-md-12">
                <div id="Carousel" class="carousel slide">
                    ...
                    <div class="carousel-inner">
                        ...
                        if ($image_popup_id) {
                            ...
                            $image_popup_url = 'http://placehold.it/900';
                        }
                        ...
                        <div class="item <?php echo $active; ?>">
                            <div class="row">
                                ...
                                <div class="col-xs-12 col-sm-12 col-md-6 <?php if ($count_items % 2 == 1) {echo 'hidden-xs hidden-sm'; }?>">
                                    <div class="row">
                                        <div class="col-xs-12 col-md-3 ">
                                            <img
                                                src="<?php echo $image_popup_url; ?>"
                                                class="img-responsive img-circle img-center" style="max-height: 300px">
                                        </div>
                                        <div class="col-xs-12 col-md-9">
                                            <blockquote>
                                                <?php
                                                $link = '/testimonial/?testimonial-view=' . get_the_ID();
                                                echo substr(get_the_content(), 0, 200) . '... ' . '<a href="' . $link . '">Read more</a>';
                                                ?>
                                                <footer class="text-center">
                                                    <?php
                                                    echo '<strong>' . get_the_title() . '</strong><br><em>' . $company_name . '</em>';
                                                    ?>
                                                </footer>
                                            </blockquote>
                                        </div>
                    ...
                    $count_items++;
                    if ($count_items == $posts->post_count && $count_items % 2 != 0) {
                    ?>
                    </div>
                </div>
                <?php
